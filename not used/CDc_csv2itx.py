...
for line in fi:
	...
	if i>=3:
		list=line.split(",")
		Voltage=list[1]
		Step_charge_capacity=list[6]
		Step_discharge_capacity=list[7]
		Step_charge_discharge_capacity=list[8]
		Cycle_time=list[11]
		Step_time=list[13]
		fo.write(str(Voltage)+'\t'+str(Step_charge_capacity)+'\t'+str(Step_discharge_capacity)+'\t'+str(Step_charge_discharge_capacity)+'\t'+str(Cycle_time)+'\t'+str(Step_time))
	fo.write('\n')
fo.write('END')
#fo.write('\n')
